...
export default async function AcoesUsPage() {
  const supabase = createClient();
  const { data } = await supabase
    .from("asset_snapshots")
    .select("*")
    .eq("asset_type", "acao_us")
  ...
  return (
    <div className="space-y-5">
      <div>
        <p className="font-display text-3xl italic text-text">Ações EUA</p>
      ...
      <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {assets.length === 0 && (
          <Card className="p-6 text-center text-sm text-text-muted sm:col-span-2 lg:col-span-3">
            Nenhuma ação US processada ainda.
          </Card>
        )}
      </div>
    </div>
